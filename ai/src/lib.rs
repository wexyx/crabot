...
use crate::agent::{
    ability::AbilityFactory, ability_tools, agent::Agent, carbot_planner, model::NodeInfo,
};
...
async fn embedding(data: &str) -> Result<Vec<u8>, Error> {
    let client = client::openai::OpenAI::new(
...
pub async fn demo() -> Result<(), Error> {
    ...
    let planner = carbot_planner::Planner::new(
        NodeInfo {
...
pub fn vec_f32_to_bytes(embedding: &Vec<f32>) -> Vec<u8> {
    ...
    // 安全地按内存复制
    unsafe {
        std::ptr::copy_nonoverlapping(embedding.as_ptr() as *const u8, bytes.as_mut_ptr(), len);
    }
